...
async function checkLoginStatus() {
  try {
    const response = await fetch('https://wake-career.jp/input-log/activity/create', {
      credentials: 'include'
    });
    const text = await response.text();
    return text.includes('ログの種類') && !text.includes('ログイン');
  } catch (error) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'registerLog') {
    registerActivityLog(request.data).then(result => {
