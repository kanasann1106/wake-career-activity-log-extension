function fillActivityLogForm() {
  ...
  setTimeout(() => {
    ...
    if (typeValue) {
      const typeRadio = document.querySelector(`input[name="activity_type_id"][value="${typeValue}"]`);
      if (typeRadio) {
        ...
        const waitForFormRender = () => {
          ...
          const checkForForm = () => {
            attempts++;
            const urlFieldAfterRender = document.querySelector('input[name="url"]');
            
            if (urlFieldAfterRender) {
              // フォームが見つかったら値を設定
              if (url) {
                urlFieldAfterRender.value = url;
                urlFieldAfterRender.dispatchEvent(new Event('input', { bubbles: true }));
                urlFieldAfterRender.dispatchEvent(new Event('change', { bubbles: true }));
              }
              
              const commentFieldAfterRender = document.querySelector('textarea[name="comment"]');
              if (commentFieldAfterRender && comment) {
                commentFieldAfterRender.value = comment;
                commentFieldAfterRender.dispatchEvent(new Event('input', { bubbles: true }));
                commentFieldAfterRender.dispatchEvent(new Event('change', { bubbles: true }));
              }
    ...
    try {
      ...
      const waitForFormAndSubmit = () => {
        ...
        const checkAndSubmit = () => {
          ...
          const submitButton = document.querySelector('input[type="submit"], button[type="submit"], .btn-submit');
          ...
          if (urlField && submitButton) {
              ...
              if (form) {
                const handleSubmit = (e) => {
                  ...
                  setTimeout(() => {
                    ...
                    } else if (document.querySelector('.alert-error, .error-message')) {
            ...
            reject(new Error('レンダリング後のフォーム要素が見つかりませんでした（タイムアウト）'));
          }
        };
...
if (window.location.hostname === 'wake-career.jp' &&
